...
u64 is_legal_perft(const Position& position, usize depth) {
  ...
  for (u32 i = 0; i < 0x10000; i++) {
    if ((i & 0xF000) >= 0xA000 && (i & 0xF000) <= 0xB000)
      continue;
    ...
    if (position.is_legal(m)) {
      ...
      result += child;
      if constexpr (print) {
        fmt::print("{}: {} // {:04x}\n", m.to_string(MoveFormat::frc), child, m.raw);
      }
    }
  }
...
int main() {
  std::vector<std::tuple<std::string_view, std::vector<u64>>> cases {{
    ...
    {"r3k2r/p1ppqpb1/1n2pnp1/3PN3/1p2P3/2N2Q1p/PPPBbPPP/R2K3R w kq - 0 2", {{1, 5}}},
    {"8/2p5/3p4/KP5r/1R3p1k/6P1/4P3/8 b - - 0 1", {{1, 4}}},
    {"r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1", {{1, 48, 2039, 97862}}},  //, 4085603
    ...
    {"r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1", {{1, 6, 264, 9467, 422333}}},
    {"rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8", {{1, 44, 1486, 62379}}},                 //, 2103487
    {"r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10", {{1, 46, 2079, 89890}}},  //, 3894594
  }};
