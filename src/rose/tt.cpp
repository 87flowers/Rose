...
namespace rose::tt {
  ...
  auto TT::tt_alloc(usize mb) -> Entry* {
    return static_cast<Entry*>(std::aligned_alloc(4096, megabytes_to_entry_count(mb) * sizeof(Entry)));
  }

  auto TT::tt_free(Entry* ptr) -> void {
    return std::free(ptr);
  }
  ...
  auto TT::load(u64 hash, int ply) const -> std::optional<LookupResult> {
  ...
  auto TT::store(u64 hash, int ply, LookupResult lr) -> void {
    const auto [index, fragment] = split_hash(m_count, hash);
    m_table.get()[index] = Entry {fragment, ply, lr};
  }
