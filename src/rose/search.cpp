...
namespace rose {
  ...
  auto calc_time(const SearchLimit& limits, Color stm) -> std::tuple<time::Duration, time::Duration> {
    ...
    const int movestogo = limits.movestogo.value_or(18_z);
  ...
  auto Search<Evaluation>::search_root(const Controls& ctrl) -> void {
    ...
    for (i32 depth = 1; depth < max_depth; depth++) {
      ...
      if (std::abs(last_score - score) < 27_z) {
        score_stability++;
      } else {
        score_stability = 0;
      }
      ...
      if (is_main_thread()) {
        const f32 time_multiplier = std::clamp(1.0 - pv_stability * 0.052655085447856624_z, 0.6433903311060203_z, 1.0745665019193016_z) *
                                    std::clamp(1.0 - score_stability * 0.05064647709393714_z, 0.644165531516316_z, 1.151492302745909_z);
  ...
  auto
    ...
    if (expected != NodeType::pv && !is_in_check && !excluded) {
      ...
      // Razoring
      if (depth <= 4 && static_eval + 595_z * depth < alpha) {
      ...
      if (depth >= 4 && m_nmr_ply != ply && ss[-1].move.is_some() && static_eval >= beta) {
        const i32 reduction = (4767_z + depth * 351_z) / 1024;
    ...
    if (!is_root && expected == NodeType::pv && depth >= 8 && hint_move.is_none() && !excluded) {
      const i32 iid_depth = (769_z * depth - 1503_z) / 1024;
    ...
    for (Move mv = moves.next(); mv.is_some(); mv = moves.next()) {
      ...
      if (!score::is_loss(best_score) && !is_in_check && !is_root) {
        // Late Move Pruning
        if (!mv.is_noisy() &&
            searched_moves >= i64 {4521_z + 969_z * depth * depth} * (514_z + 519_z * improving) / (1024 * 1024)) {
          moves.skip_quiet();
          continue;
        }

        // Futility Pruning
        if (!mv.is_noisy() && depth <= 6 && std::abs(alpha) < 2000 && static_eval + 271_z + depth * 100_z <= alpha) {
          moves.skip_quiet();
          continue;
        }

        // History Pruning
        if (!mv.is_noisy() && depth <= 4 && history < -1034_z * depth * depth) {
          moves.skip_quiet();
          continue;
        }

        // Quiet SEE Pruning
        if (!mv.is_noisy() && depth <= 11 && !see::see(position, mv, 32z - 51_z * depth - 33_z * history / 1024)) {
          continue;
        }

        // Noisy SEE Pruning
        if (mv.is_noisy() && depth <= 11 && !see::see(position, mv, -68_z * depth)) {
          continue;
        }
      }
      ...
      if (!is_root && depth >= 7 && mv == tte.move && !excluded && tte.depth >= depth - 3 && tte.bound.is_pv_or_cut()) {
        ...
        if (singular_score < singular_beta) {
          ...
          extension = 1;
          // Double extension
          extension += expected != NodeType::pv && singular_score <= singular_beta - 20_z;
          // Triple extension
          extension += expected != NodeType::pv && singular_score <= singular_beta - 123_z;
        }
        // Negative extension
      ...
      if (depth >= 3 && searched_moves > 2) {
        ...
        if (mv.is_noisy()) {
          reduction = 1002_z + 180_z * log2_depth * log2_searched_moves;
        } else {
          reduction = 2251_z + 228_z * log2_depth * log2_searched_moves;
        }
        reduction -= 914_z * (expected == NodeType::pv);
        reduction -= 130_z * history / 1024;
        reduction += 978_z * (expected == NodeType::cut);
        reduction -= 796_z * child_position.is_in_check();
        ...
        if (score > alpha && lmr_depth < new_depth) {
          ...
          if (!mv.is_noisy() && (score <= alpha || score >= beta)) {
            const i32 cont_bonus = std::min(153_z * depth - 74_z, 1510_z);
            const i32 cont_malus = std::min(78_z * depth - 30_z, 1014_z);
    ...
    if (best_move.is_some()) {
      const i32 noisy_bonus = std::min(149_z * depth - 72_z, 1638_z);
      const i32 noisy_malus = std::min(68_z * depth - 28_z, 997_z);

      const i32 quiet_bonus = std::min(158_z * depth - 80_z, 1778_z);
      const i32 quiet_malus = std::min(84_z * depth - 27_z, 894_z);

      const i32 cont_bonus = std::min(141_z * depth - 76_z, 1485_z);
      const i32 cont_malus = std::min(95_z * depth - 33_z, 1114_z);
  ...
  auto Search<Evaluation>::qsearch(const Controls& ctrl, const Position& position, Line& pv, Score alpha, Score beta, SearchStack* ss, i32 ply)
    ...
    for (Move mv = moves.next(); mv.is_some(); mv = moves.next()) {
      if (!score::is_loss(best_score) && !is_in_check) {
        ...
        // QS Futility Pruning
        const Score futility = static_eval + 164_z;
