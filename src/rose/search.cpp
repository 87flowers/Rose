...
namespace rose {
  ...
  auto calc_time(const SearchLimit& limits, Color stm) -> std::tuple<time::Duration, time::Duration> {
    ...
    const auto hard_limit = std::min<time::Milliseconds>(safe_remaining_ms / movestogo * 7_z + increment_ms * 15 / 16, safe_remaining_ms);
    const auto soft_limit = std::min<time::Milliseconds>(safe_remaining_ms / movestogo + increment_ms * 15 / 16, safe_remaining_ms);
  ...
  auto Search<Evaluation>::search_root(const Controls& ctrl) -> void {
    ...
    for (i32 depth = 1; depth < max_depth; depth++) {
      ...
      if (last_depth >= 1 && last_pv.first_move() != pv.first_move())
      ...
      if (is_main_thread()) {
        ...
        const f64 best_move_instability = 1.0 + 2.2_z * best_move_changes / static_cast<f64>(m_shared.stats.size());
        ...
        const f32 time_multiplier = std::max(2.0_z - 1.5_z * best_move_nodes / total_nodes, 0.5_z) *
                                    ...
                                    std::max(1.0 - score_stability * 0.04828987665300888_z, 0.667434450591335_z) * best_move_instability;
