...
namespace rose {
  ...
  auto calc_time(const SearchLimit& limits, Color stm) -> std::tuple<time::Duration, time::Duration> {
    ...
    const int movestogo = limits.movestogo.value_or(19_z);
  ...
  auto Search<Evaluation>::search_root(const Controls& ctrl) -> void {
    ...
    for (i32 depth = 1; depth < max_depth; depth++) {
      ...
      if (std::abs(last_score - score) < 26_z) {
        score_stability++;
      } else {
        score_stability = 0;
      }
      ...
      if (is_main_thread()) {
        const f32 time_multiplier = std::clamp(1.0 - pv_stability * 0.05168098853482218_z, 0.6769284624055887_z, 1.0771291345578324_z) *
                                    std::clamp(1.0 - score_stability * 0.05207743778290174_z, 0.6860145139122389_z, 1.0938687171760046_z);
  ...
  auto
    ...
    if (expected != NodeType::pv && !is_in_check && !excluded) {
      ...
      // Razoring
      if (depth <= 4 && static_eval + 578_z * depth < alpha) {
      ...
      if (depth >= 4 && m_nmr_ply != ply && ss[-1].move.is_some() && static_eval >= beta) {
        const i32 reduction = (4561_z + depth * 353_z) / 1024;
    ...
    if (!is_root && expected == NodeType::pv && depth >= 8 && hint_move.is_none() && !excluded) {
      const i32 iid_depth = (750_z * depth - 1433_z) / 1024;
    ...
    for (Move mv = moves.next(); mv.is_some(); mv = moves.next()) {
      ...
      if (!score::is_loss(best_score) && !is_in_check && !is_root) {
        // Late Move Pruning
        if (!mv.is_noisy() &&
            searched_moves >= i64 {4436_z + 957_z * depth * depth} * (505_z + 521_z * improving) / (1024 * 1024)) {
          moves.skip_quiet();
          continue;
        }

        // Futility Pruning
        if (!mv.is_noisy() && depth <= 6 && std::abs(alpha) < 2000 && static_eval + 260_z + depth * 99_z <= alpha) {
          moves.skip_quiet();
          continue;
        }

        // History Pruning
        if (!mv.is_noisy() && depth <= 4 && history < -975_z * depth * depth) {
          moves.skip_quiet();
          continue;
        }

        // Quiet SEE Pruning
        if (!mv.is_noisy() && depth <= 11 && !see::see(position, mv, 32z - 51_z * depth - 32_z * history / 1024)) {
          continue;
        }

        // Noisy SEE Pruning
        if (mv.is_noisy() && depth <= 11 && !see::see(position, mv, -69_z * depth)) {
          continue;
        }
      }
      ...
      if (!is_root && depth >= 7 && mv == tte.move && !excluded && tte.depth >= depth - 3 && tte.bound.is_pv_or_cut()) {
        ...
        if (singular_score < singular_beta) {
          ...
          extension = 1;
          // Double extension
          extension += expected != NodeType::pv && singular_score <= singular_beta - 19_z;
          // Triple extension
          extension += expected != NodeType::pv && singular_score <= singular_beta - 124_z;
        }
        // Negative extension
      ...
      if (depth >= 3 && searched_moves > 2) {
        ...
        if (mv.is_noisy()) {
          reduction = 997_z + 177_z * log2_depth * log2_searched_moves;
        } else {
          reduction = 2302_z + 237_z * log2_depth * log2_searched_moves;
        }
        reduction -= 928_z * (expected == NodeType::pv);
        reduction -= 129_z * history / 1024;
        reduction += 963_z * (expected == NodeType::cut);
        reduction -= 794_z * child_position.is_in_check();
        ...
        if (score > alpha && lmr_depth < new_depth) {
          ...
          if (!mv.is_noisy() && (score <= alpha || score >= beta)) {
            const i32 cont_bonus = std::min(156_z * depth - 74_z, 1531_z);
            const i32 cont_malus = std::min(78_z * depth - 28_z, 988_z);
    ...
    if (best_move.is_some()) {
      const i32 noisy_bonus = std::min(151_z * depth - 73_z, 1606_z);
      const i32 noisy_malus = std::min(70_z * depth - 30_z, 982_z);

      const i32 quiet_bonus = std::min(160_z * depth - 76_z, 1694_z);
      const i32 quiet_malus = std::min(85_z * depth - 27_z, 942_z);

      const i32 cont_bonus = std::min(143_z * depth - 74_z, 1497_z);
      const i32 cont_malus = std::min(91_z * depth - 31_z, 1119_z);
  ...
  auto Search<Evaluation>::qsearch(const Controls& ctrl, const Position& position, Line& pv, Score alpha, Score beta, SearchStack* ss, i32 ply)
    ...
    for (Move mv = moves.next(); mv.is_some(); mv = moves.next()) {
      if (!score::is_loss(best_score) && !is_in_check) {
        ...
        // QS Futility Pruning
        const Score futility = static_eval + 165_z;
