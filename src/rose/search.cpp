...
namespace rose {
  ...
  template <typename Controls> auto Search::searchRoot(const Controls &ctrl) -> void {
    const auto print_info = [this, &ctrl](i32 depth, Score score, const Line &pv) {
    ...
    for (i32 depth = 1; depth < max_search_ply; depth++) {
      Line pv{};
      const Score score = search<Score, nodetype::Root>(ctrl, m_root, pv, eval::min_score, eval::max_score, 0, depth);
  ...
  inline auto Search::isDraw(const Position &position, bool is_in_check, i32 ply) -> std::optional<Score> {
  ...
  auto Search::search(const Controls &ctrl, const Position &position, Line &pv, Score alpha, Score beta, i32 ply, i32 depth) -> Score {
    ...
    if constexpr (!NodeT::is_pv) {
      ...
      if (!is_in_check) {
        const Score static_eval = eval::hce(position);
    ...
    Score best_score = eval::no_moves;
    ...
    if (tte.bound == tt::Bound::none && depth >= tunable::iid_min_depth) {
      pv.pv[0] = Move::none();
      search<Move, NodeT>(ctrl, position, pv, alpha, beta, ply, depth - tunable::iid_reduction);
      best_move = pv.pv[0];
    }
    ...
    for (Move m = moves.next(); m != Move::none(); m = moves.next()) {
      ...
      const Score child_score = [&] {
        if (moves_searched > tunable::lmr_move_threshold && depth > tunable::lmr_depth_threshold) {
          ...
          if (reduction > 1) {
            const Score lmr_score = -search<Score, nodetype::NonPv>(ctrl, child_position, child_pv, -(alpha + 1), -alpha, ply + 1, depth - reduction);
            if (lmr_score <= alpha)
              return lmr_score;
          }
        }
        ...
        if (NodeT::is_pv && moves_searched > 0) {
          const Score scout_score = -search<Score, nodetype::NonPv>(ctrl, child_position, child_pv, -(alpha + 1), -alpha, ply + 1, depth - 1);
          if (scout_score <= alpha)
            return scout_score;
        }

        return -search<Score, typename NodeT::Next>(ctrl, child_position, child_pv, -beta, -alpha, ply + 1, depth - 1);
      }();
