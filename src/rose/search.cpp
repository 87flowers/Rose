    ...
    for (Move mv = moves.next(); mv.is_some(); mv = moves.next()) {
      ...
      if (!is_root && depth >= 7 && mv == tte.move && !excluded && tte.depth >= depth - 3 && tte.bound.is_pv_or_cut()) {
        ...
        if (singular_score < singular_beta) {
          ...
          extension = 1;
          // Double extension
          extension += expected != NodeType::pv && singular_score <= singular_beta - 10 - 30 * mv.is_noisy();
          // Triple extension
          extension += expected != NodeType::pv && singular_score <= singular_beta - 110 - 30 * mv.is_noisy();
        }
        // Negative extension
