...
namespace rose {
  ...
  inline static auto is_castle_legal(const Position& pos, Square rook, i8 rook_dest, i8 king_dest) -> bool {
  ...
  auto Position::is_legal(Move m) const -> bool {
    ...
    const bool valid_attack = m_masked_attack_table.read(m.to()).is_set(src.id());
    ...
    if (src.ptype() == PieceType::p) {
      ...
      const Bitboard pinned_quiet_pawns = m_pinned & ~Bitboard::file_mask(king.file());
